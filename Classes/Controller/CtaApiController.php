...
class CtaApiController
{
    ...
    public function updateCtaAction(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($params['secret'] == self::getSecret()) {
            $overwrite = isset($params['overwrite']) ? (bool)$params['overwrite'] : false;
            $cta = [
                // 'hubspot_updated_at' =>
            ...
            $response->withAddedHeader('Access-Control-Allow-Methods', 'OPTIONS, GET, POST');
            $response->withAddedHeader('Access-Control-Allow-Headers', 'Content-Type, Depth, User-Agent, X-File-Size, X-Requested-With, If-Modified-Since, X-File-Name, Cache-Control');*/
